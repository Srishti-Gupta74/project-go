...
function GoLogo({ size = "md", t, isDark }) {
  const [tagIdx, setTagIdx] = useState(0);
  const [animating, setAnimating] = useState(false);
  const sizes = {
    sm:  { g:18, o:18, tag:11, gap:3 },
    md:  { g:28, o:28, tag:14, gap:5 },
    lg:  { g:48, o:48, tag:20, gap:8 },
    xl:  { g:64, o:64, tag:26, gap:10 },
  };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setAnimating(true);
      setTimeout(() => {
        setTagIdx(i => (i + 1) % GO_TAGLINES.length);
        setAnimating(false);
      }, 350);
    }, 2200);
  ...
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: s.gap }}>
      {/* G */}
      <span style={{
        fontFamily: "'Fraunces',serif", fontSize: s.g, fontWeight: 900,
        color: t.green, lineHeight: 1, letterSpacing: -1,
      }}>G</span>
      {/* O = earth emoji */}
      <span style={{ fontSize: s.o * 0.85, lineHeight: 1, display: "inline-block" }}>🌍</span>
      {/* Animated tagline */}
      <span style={{
        fontFamily: "'Outfit',sans-serif", fontSize: s.tag, fontWeight: 700,
        color: t.textMid, letterSpacing: 1,
...
// ─── SDG Data ─────────────────────────────────────────────────────────────────
const SDGS = [
  { id:1,  color:"#e5243b", emoji:"🏠", title:"No Poverty",            short:"End poverty everywhere",                    how:"Recycling creates livelihoods for millions of waste pickers and kabadiwala workers across India." },
  { id:3,  color:"#4c9f38", emoji:"💚", title:"Good Health",           short:"Ensure healthy lives for all",              how:"Proper waste disposal prevents disease. Every item you sort correctly stops toxins from entering soil and water." },
  { id:6,  color:"#26bde2", emoji:"💧", title:"Clean Water",           short:"Clean water and sanitation",                how:"Hazardous waste and plastics leach into groundwater. Correct disposal protects India's rivers and wells." },
  { id:7,  color:"#fcc30b", emoji:"⚡", title:"Clean Energy",          short:"Affordable and clean energy",               how:"Recycling aluminium uses 95% less energy than making new. E-waste recycling recovers materials for solar panels." },
  { id:8,  color:"#a21942", emoji:"💼", title:"Decent Work",           short:"Inclusive growth and good jobs",            how:"India's recycling economy can create millions of formal jobs. Your kabadiwala earnings contribute directly." },
  { id:11, color:"#fd9d24", emoji:"🏙️", title:"Sustainable Cities",    short:"Inclusive, safe, resilient cities",         how:"Segregating waste at source is the #1 thing citizens can do to help India's cities achieve zero-landfill targets." },
  { id:12, color:"#bf8b2e", emoji:"♻️", title:"Responsible Consumption",short:"Sustainable consumption & production",     how:"Every scan you do tracks and reduces your waste footprint — the core of SDG 12." },
...
const GlobalStyles = () => (
  <style>{`
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .ww-upload:hover{border-color:#4ade80!important;background:rgba(74,222,128,.06)!important;}
    .ww-card-hover{transition:all .25s ease!important}
    .ww-card-hover:hover{transform:translateY(-3px)!important;box-shadow:0 16px 48px rgba(0,0,0,.25)!important}
    input{transition:border-color .2s,box-shadow .2s!important}
    input:focus{outline:none!important}
    input::placeholder{opacity:.5}
    ::-webkit-scrollbar{width:5px}
    ::-webkit-scrollbar-track{background:transparent}
    ::-webkit-scrollbar-thumb{background:rgba(74,222,128,.2);border-radius:4px}
    ...
    .ww-app-shell{ display:flex; flex-direction:column; min-height:100vh; }
    ...
    .ww-nav-tabs{
      max-width:1200px; margin:0 auto; width:100%;
      padding:0 24px 10px; display:flex; gap:6px;
    }
    ...
    .ww-sidenav-btn{
      ...
      display:flex; align-items:center; gap:12px; transition:all .2s; text-align:left;
    }

    /* Dashboard sub-tabs responsive */
    .ww-dash-tabs{ display:grid; grid-template-columns:repeat(4,1fr); gap:6px; margin-bottom:18px; }
    @media(min-width:900px){
      .ww-dash-tabs{ display:none!important; }
    }
    .ww-dash-sidenav{ display:none; }
    @media(min-width:900px){
      .ww-dash-sidenav{ display:flex!important; flex-direction:column; gap:6px; margin-bottom:16px; }
    }

    /* Charts grid on desktop */
    @media(min-width:900px){
      .ww-charts-grid{ display:grid!important; grid-template-columns:1fr 1fr; gap:14px; }
      .ww-charts-grid > *{ margin-bottom:0!important; }
      .ww-charts-full{ grid-column:1/-1; }
    }
    ...
    .ww-auth-tagline-light{ background:rgba(22,163,74,.06)!important; border-color:rgba(22,163,74,.15)!important; }
  `}</style>
);
...
function AuthPage({onLogin, isDark, toggleDark}) {
  ...
  return (
    <div className="ww-auth-wrap" style={{background:t.bg,position:"fixed",inset:0,overflow:"auto",transition:"background .4s"}}>
      <GlobalStyles/>
      ...
      <button onClick={toggleDark} style={{position:"fixed",top:20,right:20,zIndex:10,width:44,height:24,borderRadius:12,background:isDark?"rgba(74,222,128,.2)":"rgba(22,163,74,.15)",border:`1px solid ${t.borderGreen}`,cursor:"pointer",display:"flex",alignItems:"center",padding:"2px",transition:"all .3s"}}>
      ...
      {/* LEFT — hero (desktop only shows taglines) */}
      <div className="ww-auth-hero" style={{textAlign:"center",marginBottom:32,zIndex:1}}>
        <div style={{position:"relative",display:"inline-block",marginBottom:20}}>
          <div style={{width:88,height:88,borderRadius:"50%",background:`linear-gradient(135deg,${isDark?"#052e16":"#dcfce7"},${isDark?"#0d4a1a":"#bbf7d0"})`,border:`2px solid ${t.borderGreen}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,margin:"0 auto",boxShadow:`0 0 60px rgba(74,222,128,${isDark?.25:.3})`,animation:"ww-pulse 3s ease-in-out infinite"}}>♻️</div>
          <div style={{position:"absolute",inset:-8,borderRadius:"50%",border:`1.5px solid ${t.borderGreen}`,animation:"ww-ring 2.5s infinite"}}/>
          <div style={{position:"absolute",inset:-18,borderRadius:"50%",border:`1px solid ${t.borderGreen}`,animation:"ww-ring 2.5s infinite",animationDelay:".5s"}}/>
        </div>
        <div style={{margin:"0 0 8px"}}><GoLogo size="xl" t={t} isDark={isDark}/></div>
        <p style={{color:t.textMid,fontSize:12,letterSpacing:2.5,textTransform:"uppercase",fontFamily:"'Outfit',sans-serif",fontWeight:500}}>Every scan saves the planet</p>
        <div className="ww-auth-taglines" style={{marginTop:32}}>
          {[{icon:"📸",text:"Scan any waste with your camera"},{icon:"🤖",text:"AI identifies & classifies instantly"},{icon:"🌱",text:"Earn EcoCoins, plant real trees"},{icon:"💰",text:"Track real money from recycling"}].map((item,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 16px",borderRadius:14,background:isDark?"rgba(74,222,128,.06)":"rgba(22,163,74,.06)",border:`1px solid ${t.borderGreen}`}}>
              <span style={{fontSize:22}}>{item.icon}</span>
              <span style={{fontSize:14,color:t.textMid,fontFamily:"'Outfit',sans-serif",fontWeight:500}}>{item.text}</span>
            </div>
          ))}
          <p style={{color:t.textDim,fontSize:13,marginTop:16,fontFamily:"'Fraunces',serif",fontStyle:"italic",lineHeight:1.7}}>"The Earth does not belong to us.<br/>We belong to the Earth."</p>
        </div>
      </div>

      {/* RIGHT — form */}
      <div className="ww-auth-form" style={{width:"100%",maxWidth:420,zIndex:1,animation:"ww-slideup .6s ease"}}>
        <Card t={t} style={{padding:"28px",borderRadius:28}} glow>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            {mode==="signup"&&<div>
              ...
              <input value={name} onChange={e=>setName(e.target.value)} placeholder="Aarav Sharma" onKeyDown={e=>e.key==="Enter"&&submit()} style={{width:"100%",padding:"13px 16px",background:isDark?"rgba(0,0,0,.5)":"rgba(255,255,255,.9)",border:`1.5px solid ${t.border}`,borderRadius:12,color:t.text,fontSize:14,fontFamily:"'Outfit',sans-serif"}}/>
            </div>}
            <div>
              ...
              <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" type="email" onKeyDown={e=>e.key==="Enter"&&submit()} style={{width:"100%",padding:"13px 16px",background:isDark?"rgba(0,0,0,.5)":"rgba(255,255,255,.9)",border:`1.5px solid ${t.border}`,borderRadius:12,color:t.text,fontSize:14,fontFamily:"'Outfit',sans-serif"}}/>
            </div>
            <div>
...
//  SDG PAGE
// ═══════════════════════════════════════════════════════════════════════════════
function SDGPage({ t, isDark }) {
  const [selected, setSelected] = useState(null);
  return (
    <div style={{ paddingBottom: 20 }}>
      <div style={{ background: `linear-gradient(135deg,${isDark?"#052e16":"#dcfce7"},${isDark?"#0a2a0a":"#bbf7d0"})`, border: `1.5px solid ${t.borderGreen}`, borderRadius: 24, padding: "22px 20px", marginBottom: 16, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10, marginBottom: 16 }}>
        {SDGS.map(sdg => (
          <button key={sdg.id} onClick={() => setSelected(selected?.id === sdg.id ? null : sdg)}
            style={{ padding: "14px 10px", borderRadius: 16, border: `2px solid ${selected?.id === sdg.id ? sdg.color : sdg.color+"40"}`, background: selected?.id === sdg.id ? `${sdg.color}18` : isDark ? "rgba(255,255,255,.02)" : "rgba(255,255,255,.7)", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 6, transition: "all .2s" }}>
            <span style={{ fontSize: 26 }}>{sdg.emoji}</span>
            <div style={{ fontSize: 9, fontWeight: 800, color: sdg.color, fontFamily: "'Outfit',sans-serif", textAlign: "center", letterSpacing: .5 }}>SDG {sdg.id}</div>
            <div style={{ fontSize: 10, color: t.text, fontFamily: "'Outfit',sans-serif", fontWeight: 600, textAlign: "center", lineHeight: 1.3 }}>{sdg.title}</div>
          </button>
        ))}
      </div>
      {selected && (
        <div style={{ animation: "ww-slideup .3s ease", marginBottom: 16 }}>
          <div style={{ background: isDark ? "rgba(0,0,0,.4)" : "rgba(255,255,255,.9)", border: `2px solid ${selected.color}50`, borderRadius: 22, padding: "20px", backdropFilter: "blur(12px)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
              <div style={{ width: 56, height: 56, borderRadius: 16, background: `${selected.color}20`, border: `2px solid ${selected.color}40`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, flexShrink: 0 }}>{selected.emoji}</div>
              <div>
                <div style={{ fontSize: 10, color: selected.color, letterSpacing: 2, fontWeight: 700, fontFamily: "'Outfit',sans-serif" }}>SDG {selected.id}</div>
                <div style={{ fontFamily: "'Fraunces',serif", fontSize: 20, fontWeight: 900, color: t.text, lineHeight: 1.1 }}>{selected.title}</div>
                <div style={{ fontSize: 12, color: t.textMid, fontFamily: "'Outfit',sans-serif" }}>{selected.short}</div>
              </div>
            </div>
            <div style={{ background: `${selected.color}10`, border: `1px solid ${selected.color}30`, borderRadius: 14, padding: "14px" }}>
              <div style={{ fontSize: 10, color: selected.color, letterSpacing: 2, fontWeight: 700, fontFamily: "'Outfit',sans-serif", marginBottom: 8 }}>🌱 HOW YOU HELP</div>
              <p style={{ fontSize: 14, color: t.text, fontFamily: "'Outfit',sans-serif", lineHeight: 1.7, margin: 0 }}>{selected.how}</p>
            </div>
          </div>
        </div>
      )}
      <div style={{ textAlign: "center", padding: "16px", background: isDark ? "rgba(74,222,128,.04)" : "rgba(22,163,74,.04)", borderRadius: 16, border: `1px solid ${t.borderGreen}` }}>
        <div style={{ fontSize: 20, marginBottom: 8 }}>🌐</div>
        <p style={{ fontSize: 12, color: t.textDim, fontFamily: "'Outfit',sans-serif", lineHeight: 1.7, margin: 0 }}>Learn more at <strong style={{ color: t.green }}>sdgs.un.org</strong></p>
      </div>
    </div>
  );
}
...
export default function ProjectGo() {
  ...
  useEffect(()=>{
    document.body.style.background = isDark ? "#030a03" : "#f0faf0";
    document.documentElement.style.background = isDark ? "#030a03" : "#f0faf0";
  },[isDark]);
  ...
  return (
    <div style={{minHeight:"100vh",width:"100%",background:t.bg,fontFamily:"'Outfit',sans-serif",color:t.text,position:"relative",transition:"background .4s, color .4s"}}>
      <GlobalStyles/>
      <BackgroundOrbs isDark={isDark}/>
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:t.navBg,backdropFilter:"blur(20px)",borderBottom:`1px solid ${t.border}`,transition:"background .4s"}}>
        ...
        <div className="ww-nav-inner">
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:38,height:38,borderRadius:"50%",background:`linear-gradient(135deg,${isDark?"#052e16":"#dcfce7"},${isDark?"#0d4a1a":"#bbf7d0"})`,border:`1.5px solid ${t.borderGreen}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,boxShadow:`0 0 16px ${t.green}30`}}>♻️</div>
            <div>
              <div style={{fontFamily:"'Fraunces',serif",fontSize:20,fontWeight:900,color:t.green,lineHeight:1}}><GoLogo size="sm" t={t} isDark={isDark}/></div>
              <div style={{fontSize:10,color:t.textDim,letterSpacing:.5,fontFamily:"'Outfit',sans-serif"}}>Hi, {user.name.split(" ")[0]}! 👋</div>
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{padding:"6px 14px",background:isDark?"rgba(251,191,36,.12)":"rgba(217,119,6,.1)",border:`1px solid ${isDark?"rgba(251,191,36,.25)":"rgba(217,119,6,.2)"}`,borderRadius:30,fontSize:13,fontWeight:700,color:isDark?"#fbbf24":"#d97706",fontFamily:"'Outfit',sans-serif",cursor:"pointer"}} onClick={()=>setPage("rewards")}>🪙 {totalPts} EcoCoins</div>
            <button onClick={toggleDark} style={{width:46,height:24,borderRadius:12,background:isDark?"rgba(74,222,128,.2)":"rgba(22,163,74,.15)",border:`1px solid ${t.borderGreen}`,cursor:"pointer",display:"flex",alignItems:"center",padding:"3px",transition:"all .3s",flexShrink:0}}>
            ...
            <button onClick={()=>setShowSignOut(true)} style={{padding:"6px 14px",borderRadius:10,background:isDark?"rgba(248,113,113,.1)":"rgba(220,38,38,.07)",border:`1px solid ${t.red}30`,color:t.red,cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:"'Outfit',sans-serif"}}>Sign Out</button>
          </div>
        </div>
        {/* Mobile tabs */}
        <div className="ww-nav-tabs">
          {[{id:"scan",icon:"📸",label:"Scan"},{id:"earnings",icon:"💰",label:"Earnings"},{id:"rewards",icon:"🎁",label:"Rewards"},{id:"sdg",icon:"🌍",label:"SDGs"},{id:"dashboard",icon:"📊",label:"Stats"}].map(pg=>(
      ...
      <div className="ww-main-layout">

        {/* Desktop sidebar nav */}
        <div className="ww-sidebar">
          <div style={{fontFamily:"'Fraunces',serif",fontSize:12,fontWeight:700,color:t.textDim,letterSpacing:2,marginBottom:8,paddingLeft:4}}>NAVIGATE</div>
          {[{id:"scan",icon:"📸",label:"Scan Waste",desc:"AI waste identifier"},{id:"earnings",icon:"💰",label:"Earnings",desc:"Track recycling money"},{id:"rewards",icon:"🎁",label:"Rewards",desc:"Redeem EcoCoins"},{id:"sdg",icon:"🌍",label:"SDG Goals",desc:"UN sustainability goals"},{id:"dashboard",icon:"📊",label:"Dashboard",desc:"Stats & badges"}].map(pg=>(
            <button key={pg.id} className="ww-sidenav-btn" onClick={()=>setPage(pg.id)}
              style={{background:page===pg.id?`linear-gradient(135deg,${t.greenDeep}22,${t.green}18)`:"transparent",color:page===pg.id?t.green:t.textMid,border:page===pg.id?`1px solid ${t.borderGreen}`:`1px solid transparent`,position:"relative"}}>
              <span style={{fontSize:22,flexShrink:0}}>{pg.icon}</span>
              <div style={{flex:1}}>
          ...
          {/* Sidebar stats */}
          <div style={{marginTop:16,padding:"16px",background:isDark?"rgba(74,222,128,.05)":"rgba(22,163,74,.05)",border:`1px solid ${t.borderGreen}`,borderRadius:16}}>
            <div style={{fontSize:10,color:t.textDim,letterSpacing:2,fontFamily:"'Outfit',sans-serif",fontWeight:700,marginBottom:12}}>YOUR IMPACT</div>
            {[
              {label:"EcoCoins",val:`🪙 ${totalPts}`,color:isDark?"#fbbf24":"#d97706"},
            ].map((s,i)=>(
        ...
        <div className="ww-right-panel">
          <Card t={t} style={{padding:"20px",borderRadius:20,marginBottom:14}}>
            <div style={{fontSize:10,color:t.green,letterSpacing:2,fontWeight:700,fontFamily:"'Outfit',sans-serif",marginBottom:14}}>🌍 INDIA WASTE FACTS</div>
            {[
              {stat:"62M", label:"tonnes of waste generated yearly"},
              {stat:"80%", label:"ends up in open dumps or rivers"},
              {stat:"₹1L Cr", label:"potential recycling economy"},
              {stat:"1 scan", label:"= 1 step toward change"},
            ].map((f,i)=>(
              <div key={i} style={{marginBottom:14,paddingBottom:14,borderBottom:i<3?`1px solid ${t.border}`:"none"}}>
                <div style={{fontFamily:"'Fraunces',serif",fontSize:26,fontWeight:900,color:t.green,lineHeight:1,marginBottom:3}}>{f.stat}</div>
                <div style={{fontSize:12,color:t.textMid,fontFamily:"'Outfit',sans-serif",lineHeight:1.4}}>{f.label}</div>
              </div>
            ))}
          </Card>
          <Card t={t} style={{padding:"16px",borderRadius:18}}>
            <div style={{fontSize:10,color:t.green,letterSpacing:2,fontWeight:700,fontFamily:"'Outfit',sans-serif",marginBottom:12}}>⚡ QUICK TIP</div>
            <p style={{fontSize:13,color:t.textMid,fontFamily:"'Outfit',sans-serif",lineHeight:1.6,margin:0}}>Dry waste like plastic bottles are worth more when clean. Rinse before recycling to get a better kabadiwala rate! 💧</p>
          </Card>
        </div>
