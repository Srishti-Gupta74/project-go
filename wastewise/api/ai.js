...
const PROMPTS = {
  ...
  centers: (itemName, category, city) => {
    ...
    return {
      contents: [{
        parts: [{
          text: `Find up to 3 REAL, VERIFIED recycling/disposal centers or scrap dealers (kabadiwalas) for "${itemName}" (${category} waste) strictly located inside "${city}", India.\nCRITICAL RULE: DO NOT MAKE UP OR INVENT FICTIONAL OR PLAUSIBLE STORES under any circumstances! Every single center, kabadiwala, or dealer you return MUST be a real, genuine business that exists on Google Maps specifically inside "${city}".\nIMPORTANT REQUIREMENT: ALL returned centers MUST be inside or within 10 km of "${city}". Under NO circumstances should you list centers from other cities or distant states across India.\nLook for: ${searchHints[category] || "recycling center"}.\nRespond ONLY with a valid JSON array — no markdown, no backticks:\n[\n  {\n    "name": "REAL verified center or kabadiwala name in ${city}",\n    "type": "type of facility",\n    "address": "REAL verifiable address in ${city}, India",\n    "distance": "approximate distance like ~1.5 km or ~3 km",\n    "phone": "verified phone number or null",\n    "hours": "verified opening hours like 9 AM - 6 PM or null",\n    "tip": "one helpful tip for visiting or calling",\n    "mapsQuery": "exact business name and location query for Google Maps"\n  }\n]\nIf you only know 1 or 2 real verified stores in "${city}", return ONLY those 1 or 2 real ones. If you cannot verify any real businesses or kabadiwalas specifically inside "${city}", return an empty JSON array [] instead of making up fake names.`
        }]
      }],
      generationConfig: { responseMimeType: "application/json", maxOutputTokens: 800 }
    };
