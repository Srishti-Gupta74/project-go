...
const PROMPTS = {
  ...
  centers: (itemName, category, city) => {
    ...
    return {
      contents: [{
        parts: [{
          text: `Find 3 real or highly realistic recycling/disposal centers or scrap dealers (kabadiwalas) for "${itemName}" (${category} waste) strictly and specifically located inside "${city}", India.\nIMPORTANT REQUIREMENT: ALL 3 centers MUST be within or very close to the specific city/area of "${city}". Under NO circumstances should you list centers from multiple different cities or distant states across India. Every single address must be inside "${city}".\nLook for: ${searchHints[category] || "recycling center"}.\nRespond ONLY with a valid JSON array — no markdown, no backticks:\n[\n  {\n    "name": "specific local center or dealer name inside ${city}",\n    "type": "type of facility",\n    "address": "full realistic local address in ${city}, India with local area name",\n    "distance": "approximate distance like ~1.5 km or ~3 km",\n    "phone": "phone number or null",\n    "hours": "opening hours like 9 AM - 6 PM or null",\n    "tip": "one helpful tip for visiting or calling",\n    "mapsQuery": "search query for Google Maps specifically in ${city}"\n  }\n]\nIf exact real names are hard to verify, generate 3 highly plausible and accurate-sounding local centers situated specifically inside the neighborhoods of "${city}".`
        }]
      }],
      generationConfig: { responseMimeType: "application/json", maxOutputTokens: 800 }
    };
